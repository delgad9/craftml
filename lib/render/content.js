    ...
    if ($scope.parent && $scope.caller){
        ...
        if ('name' in $element.attribs){
            ...
            if (m && m.type == 'tag' && m.children){
                return renderElementList($solid, m.children, contentScope)
            }
            ...
            return renderElementList($solid, contentElements, contentScope)
        }

    }

    return render_default_content($solid, $element.children, $scope)
}
