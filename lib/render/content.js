    ...
    if ('name' in element.attribs){
        ...
        if (scope.parent && scope.caller){
            ...
            if (m){
                ...
                return render_default_content(render, element.children, scope)
            }
        } else {
        ...
        if (scope.parent && scope.caller) {
            ...
            // console.log(scope.parent.parent.parent.parameters)
            // console.log(scope.parent.parent.parameters)
            // console.log(scope.parent.parameters)
            // console.log(scope.parameters)
            // scope.parent.parent.solid.pp()
            // <content> --> <foo>
            // two levels up?

            // console.log('callerScope.params', scope.callerScope.parameters)

            // must inject scope parameters from the caller's scope
            let callerScope = scope.parent.parent
            _.defaults(scope.parameters, callerScope.parameters)

            // console.log('content: scope.parent', scope.parent)
            // return renderElementList(notParameters, effectiveScope)
            //return renderElementList(notParameters, scope)

            let savedParameters = scope.parameters

            // temporarily switch params
            scope.parameters = scope.callerScope.parameters

            return renderElementList(notParameters, scope)
                .then(()=>{
                    scope.parameters = savedParameters
                })
...
function render_default_content(render, elements, scope){
