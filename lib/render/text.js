...
function load_font(fontName, context){

    let cdn = context.engine.cdn
    let fontUrl = `${cdn}/fonts/${fontName}.ttf`
...
export default function render_text($scope, $) {
    ...
    console.log('data',this.src.name, this.src.data)
    ...
    // console.log(this.src.context)

    return load_font_memoized(fontName, this.src.context)
        .then(font => {
