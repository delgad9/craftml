...
export default function solidify($solid, $element, $scope) {
    ...
    return renderElementList($solid, $element.children, $scope)
        .then(() => {
            ...
            $solid.fitToChildren()
            $solid.applyStyleRecursively($scope.css)
        })
}
...
function solidify_solids(solid){
    ...
    // convert all concav polygons into triangles
    // _.forEach(shapes, s => {
    //     if (!s.csg.polygons[0].isConvex()){
    //         s.csg.polygons = _convert_to_triangles(s)
    //     }
    // })
