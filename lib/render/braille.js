...
export default function render_braille(render, element, scope){

    let text = element.children[0].data.toUpperCase().trim()
...
var dotcsg = _.memoize(() => {
    let c = new $$$.CSG.sphere({radius: 0.5,
            resolution: 15,
            center:[0.5,0.5,0]})
    c = c.subtract($$$.cube([1,1,0.5]).translate([0,0,-0.5]))
            .scale([1.2,1.2,1])
    return c
})
