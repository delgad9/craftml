...
export default function render(element, scope) {
    ...
    // element.style =
    // style = scope.computeStyle(element)
    ...
    return  Promise
        ...
        .then(() => {
            ...
            // some descendents may not have their styles set when they were
            // thus, we recursively go down the tree to compute the styles of those
            // without any style information
            function computeStyleRecursively(s){
                if (_.isUndefined(s.style)){
                    s.computeStyle(scope.css)
                }
                _.forEach(s.children, computeStyleRecursively)
            }
            computeStyleRecursively(solid)
