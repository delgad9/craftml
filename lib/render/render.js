    ...
    return  Promise
        ...
        .then(() => {
            ...
            if (solid.name == 'text'){
                // this is a hack
                //
                // self transform can not be called before the final
                // fitToChildren()
                //
                // fitToChildren() would mess up self transform
                solid.translate(0,10,0)
            }
...
function _render_element(element, scope){
    ...
    if (r){
        return r.func.call(scope.solid, element, scope)

    } else if (element.type === 'tag'){

        if (_.includes(['h1','h2','h3','h4','h5'], element.name)){

            return _div(element, scope)

        } else {
            if (element.name == 'char'){
                console.time(element.name)
            }
            return _tag(element, scope)
                .then(solids =>{
                    if (element.name == 'char'){
        ...
        return _script(element, scope)

    } else if (element.type === 'text'){
        // console.log('rendering text')
        return _text(element, scope)
