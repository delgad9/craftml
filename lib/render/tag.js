...
export default function render_tag(render, element, scope) {
    ...
    if (_.some(element.children, _.isUndefined)){
        // console.log(element.children)
    // /    console.log(_.compact(element.children))
    }

    // _.compact is used because children can contain undefined elements

    //_.forEach(_.compact(element.children), function(c){
    ...
    // are we merging this node?
    // console.log('merging', toRender.attribs)
    if (toRender.attribs['grouping'] == 'off'){
        scope.solid.role = 'merge'
        // console.log('merging', toRender.attribs['role'])
    }
