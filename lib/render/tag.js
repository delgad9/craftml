...
export default function render_tag(render, element, scope) {
    ...
    let selectedCSSRules =
        _.filter(scope.css, r => { return CSSselect.is(element, r.selectors.join(','))})
    ...
    // console.log('css rules', scope.css.length, selectedCSSRules.length,
    //     _.pluck(selectedCSSRules, 'selectors'), 'computed style:', computedStyle)
...
_add_attribute_processor(['layout'], require('./attribs/layout'), 'each')
