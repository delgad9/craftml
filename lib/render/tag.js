...
export default function render_tag($scope) {

    // console.log(`rendering <${element.name}> with params=${JSON.stringify(scope.parameters)}`)

    var name = this.src.name

    var craft = $scope.crafts[name]
    var part = $scope.parts[name]
    ...
    var params = _resolveAttributesToParameters($scope, this.src.attribs, toRender)
    if (craft){
    ...
    toRender.parent = this

    childScope.element = this
