...
export default function render_div(render, element, scope) {
    ...
    let style = scope.solid.style//computeStyle(element)
    ...
    return renderElementList(element.children, innerScope)
        .then(() => {
            ...
            let solids = scope.solid.children

            let $ = q(scope.solid)


            // console.log($(scope.solid).find('word').length, 'words')

            let words = []
            $(scope.solid).find('word').each(function(){
                words.push(this)
            })

            // console.log('children', solids.length)
            ...
            paragraphSolid.translate(0,10,0)
            scope.solid.add(paragraphSolid)
