...
Solid.prototype.debug = function(){
    return {
        getPolygonsBoundingBox: function(){

            var b = this.csg.getBounds()

            return {
                location: new Location(b[0].x,b[0].y,b[0].z),
                size: new Size(b[1].x-b[0].x,b[1].y-b[0].y,b[1].z-b[0].z)
            }
...
function _location_partial(func){
    return function(){
        ...
        return func.apply(this, [loc])//.bind//_.partial(func, loc)()
    }
}
...
function translate(loc){
    ...
    _.forEach(['x', 'y', 'z'], function(dim) {
        this.layout.location[dim] += loc[dim]
    }, this)
    // this.layout.location = new Location(newLoc.x,newLoc.y,newLoc.z)
...
function translateTo(loc){
    ...
    var delta = new Location(d.x, d.y, d.z)
    this.m = this.m.multiply($$$.CSG.Matrix4x4.translation([delta.x, delta.y, delta.z]))
...
Solid.prototype.scaleTo = function(newSize) {
...
Solid.prototype.transformEval = function(code, params){
    ...
    var scaleTo = function(x,y,z){
        this.scaleTo({x:x, y:y, z:z})
    }.bind(this)
    ...
    var centerAt = function(x,y,z){
        this.centerAt({x:x, y:y, z:z})
    }.bind(this)
