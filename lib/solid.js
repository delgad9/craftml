...
Solid.prototype.fitToCSG = function() {
    this.layout = computeLayout(this.csg)
}
...
Solid.prototype.translate = function(loc){
    //this.m = $$$.CSG.Matrix4x4.translation([loc.x, loc.y, loc.z]).multiply(this.m)
...
function _applyTranformation(node, matrix) {
    ...
    if (_.isArray(node)) {
        ...
        if (solid.m){
            ...
            // m = m.multiply(solid.m)//.multiply(m)      
        }
...
function _applyTranformation1(node, matrix) {
