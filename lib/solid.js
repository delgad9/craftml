...
// deprecated
Box.prototype.transform_apply = function(m, n){
    var o = this.location
    var s = this.size
    var c = $$$.cube([s.x,s.y,s.z]).translate([o.x,o.y,o.z])
    c = c.transform(m)
...
Box.prototype.revert = function(){
    if (this.n){
        console.log('reverting', JSON.stringify(this.n))
...
// Convert the box to the coordinate system of
// a destination solid. The destination solid must be
// an ancestor of the source solid
Box.prototype.convertCoordinateSystemTo = function(dest){
    this.ancestor = dest
    if (this.m && this.n){
        return this.m.multiply(tm).multiply(this.n)
    } else {
...
Solid.prototype.convertCoordinateTo = function(ancestor){
    ...
    if (this.layout_reference){
        ...
        if (ancestor === this){
            ...
            if (qr){
                this.layout.transform_apply(qr.n)
            }
            if (pr){
                this.layout.transform_apply(pr.m)
            }
        ...
        if (r){
            this.layout.transform_apply(r.m)
            this.layout_reference = ancestor
        }
    }
}
...
Solid.prototype.fitToChildren = function() {
    ...
    if (this.children) {
        ...
        this.box.size = new Size(
            xrange.max - xrange.min,
            yrange.max - yrange.min,
            zrange.max - zrange.min
        )

        this.box.location = new Location(
            xrange.min,
            yrange.min,
            zrange.min
        )
...
function _location_partial(func){
    return function(){
        ...
        var ret = func.apply(this, [loc, grouped])
        ...
        if (this.parent && this.parent.parent){
            this.parent.parent.fitToChildren()
        }
...
function translateTo(loc, grouped){
    ...
    if (grouped){
        console.log('need to move the group')
        console.log('translateTo', loc)
        ...
        //var p1 = p.multiply(this.parent.m).multiply(this.m)
        // var p1 = this.m.multiply(this.parent.m).multiply(p)//p.multiply(this.parent.m).multiply(this.m)
        ...
        root.layout.transform_apply(p)
        // console.log('ROOT BOX', JSON.stringify(root.layout))

        this.layout.location = new Location(loc.x,loc.y,loc.z)
        // console.log('LOCATION', this.layout.location)
