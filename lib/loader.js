...
export function get(url, context){
    ...
    return function() {
        ...
        if (context.local){
            // console.log('loading locally', context, url)
            return fs.readFileAsync(url, 'binary')
                .then(contents => {
...
function _getContentsPromise(context, srcUrl) {
    ...
    if (context.local){
        ...
        console.log('loading locally', context, srcUrl)

        return fs.readFileAsync(srcUrl, 'binary')
            .then(contents => {
                return contents
            })
    }
...
export function loadModule(context, moduleId) {
    ...
    if (toks[0].match(/^@gist/)){
        ...
        return _getContentsPromise(context, url)
            ...
            // var url = 'http://craftml.io/raw/module/' + moduleId
