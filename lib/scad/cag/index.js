...
CAG.fromPoints = function(points) {
	...
	var sides = [];
	var prevpoint = new G.Vector2D(points[numpoints - 1]);
	var prevvertex = new CAG.Vertex(prevpoint);
	points.map(function(p) {
		var point = new G.Vector2D(p);
...
// [ab][x] = [u]
// [cd][y]   [v]
 function _solve2Linear(a, b, c, d, u, v) {
	var det = a * d - b * c;
	var invdet = 1.0 / det;
	var x = u * d - b * v;
	var y = -u * c + a * v;
...
CAG.linesIntersect = function(p0start, p0end, p1start, p1end) {
	if(p0end.equals(p1start) || p1end.equals(p0start)) {
		...
		var alphas = _solve2Linear(-d0.x, d1.x, -d0.y, d1.y, p0start.x - p1start.x, p0start.y - p1start.y);
...
CAG.prototype = {
	...
	toPolygon: function(){
		...
		var vertices = [new Vertex(firstSide.vertex0.pos.toVector3D(0))]
		this.sides.forEach(function(side) {
			vertices.push(new Vertex(side.vertex1.pos.toVector3D(0)))
		});
		var poly = new Polygon(vertices)
		return poly
	},

	toCSGPolygon: function(){
		var CSG = require('../csg')
		var Vertex = require('../geometry/Vertex'),
			Polygon = require('../geometry/Polygon')
		var firstSide = this.sides[0]
   ...
   center: function(c) {
	...
	var offsetvector = optionParser.parseAs3DVector(options, "offset", [0,0,1]);
	var twistangle = optionParser.parseAsFloat(options, "twistangle", 0);
	var twiststeps = optionParser.parseAsInt(options, "twiststeps", 10);
	...
	for(var step=0; step <= twiststeps; step++) {
		...
		var translatevector = new G.Vector2D(offsetvector.x, offsetvector.y).times(stepfraction);
		transformedcag = transformedcag.translate(translatevector);
		var bounds = transformedcag.getBounds();
		bounds[0] = bounds[0].minus(new G.Vector2D(1,1));
		bounds[1] = bounds[1].plus(new G.Vector2D(1,1));
		var stepz = offsetvector.z * stepfraction;
		if( (step == 0) || (step == twiststeps) ) {
			// bottom or top face:
			var csgshell = transformedcag.toCSG(stepz-1, stepz+1);
			var csgplane = CSG.fromPolygons([new G.Polygon([
				new G.Vertex(new G.Vector3D(bounds[0].x, bounds[0].y, stepz)),
				new G.Vertex(new G.Vector3D(bounds[1].x, bounds[0].y, stepz)),
				new G.Vertex(new G.Vector3D(bounds[1].x, bounds[1].y, stepz)),
				new G.Vertex(new G.Vector3D(bounds[0].x, bounds[1].y, stepz))
			])]);
			var flip = (step == 0);
		...
		if(step > 0) {
			...
			for(var sideindex = 0; sideindex < numsides; sideindex++) {
				...
				var p1 = new G.Polygon([
					new G.Vertex(thisside.vertex1.pos.toVector3D(stepz)),
					new G.Vertex(thisside.vertex0.pos.toVector3D(stepz)),
					new G.Vertex(prevside.vertex0.pos.toVector3D(prevstepz))
				]);
				var p2 = new G.Polygon([
					new G.Vertex(thisside.vertex1.pos.toVector3D(stepz)),
					new G.Vertex(prevside.vertex0.pos.toVector3D(prevstepz)),
					new G.Vertex(prevside.vertex1.pos.toVector3D(prevstepz))
				]);
				if(offsetvector.z < 0) {
					p1 = p1.flipped();
					p2 = p2.flipped();
