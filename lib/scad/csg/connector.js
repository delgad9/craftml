...
function Connector(point, axisvector, normalvector) {
    this.point = new G.Vector3D(point);
    this.axisvector = new G.Vector3D(axisvector).unit();
    this.normalvector = new G.Vector3D(normalvector).unit();
};
...
Connector.prototype = {
    ...
    getTransformationTo: function(other, mirror, normalrotation) {
        ...
        // shift to the origin:
        var transformation = G.Matrix4x4.translation(this.point.negated());
        // construct the plane crossing through the origin and the two axes:
        var axesplane = G.Plane.anyPlaneFromVector3Ds(
            new G.Vector3D(0, 0, 0), us.axisvector, other.axisvector);
        var axesbasis = new G.OrthoNormalBasis(axesplane);
        ...
        transformation = transformation.multiply(G.Matrix4x4.rotationZ(rotation));
        transformation = transformation.multiply(axesbasis.getInverseProjectionMatrix());
        var usAxesAligned = us.transform(transformation);
        // Now we have done the transformation for aligning the axes.
        // We still need to align the normals:
        var normalsplane = G.Plane.fromNormalAndPoint(other.axisvector, new G.Vector3D(0, 0, 0));
        var normalsbasis = new G.OrthoNormalBasis(normalsplane);
        ...
        transformation = transformation.multiply(G.Matrix4x4.translation(other.point));
        var usAligned = us.transform(transformation);
        return transformation;
    },

    axisLine: function() {
        return new G.Line3D(this.point, this.axisvector);
    },
