...
function _parse_craft(node, context) {
    if (node.attribs.module) {
        ...
        // console.log('loading %s, context', src, context)
        var filepath = path.resolve(context.basePath, src)
        return fs.readFileAsync(filepath, 'utf8')
            .then(function(contents) {                

                context.basePath = path.dirname(path.resolve('.',src))
    ...
    if ('length' in arg) {
        ...
        if (node.type === 'tag') {
            ...
            if (node.name === 'craft' && (node.attribs.src || node.attribs.module)) {
                ...
                return _parse_craft(node, context)
