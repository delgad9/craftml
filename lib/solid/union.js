...
function checkOverlap(csg1, csg2) {
    if((csg1.polygons.length === 0) || (csg2.polygons.length === 0)) {
        return false;
    } else {
        var mybounds = csg1.getBounds();
        var otherbounds = csg2.getBounds();
     ...
     // use <= => to treat "touching" as if they are not overlapping
...
// used for export only, solids that are merely adjacent are not joined
// it's fine for printing, but can not be used for further csg operations
export function unionForExport(csgs){
    // mayOverlap
    ...
    _.forEach(ps, function(p, i) {
        ...
        _.forEach(ps.slice(i), function(other, j) {
            ...
            if (p !== other && checkOverlap(p.csg, other.csg)) {
                // console.log(j)
