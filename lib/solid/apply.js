...
function _applyTranformation(node, matrix) {
    ...
    if (_.isArray(node)) {
        ...
        if (solid.csg && solid.m) {
            ...
            let isUnity = _.isEqual(m.elements, Matrix4x4.unity().elements)
            let tooManyPolygons = solid.csg.polygons.length > 1000
            if (isUnity || tooManyPolygons){
                // solid.csg.polygons.length > 1000){
                // console.log('large, no transform')
                // console.log(m)
                solid.m = m
