...
function _applyTranformation(solid, matrix, flipped) {
    ...
    if (solid.csg){
        ...
        let isUnity = _.isEqual(matrix.elements, Matrix4x4.unity().elements)
        if (!isUnity){
            if (solid.csg.polygons.length < 2000){

                // console.log(solid.parent.name, 'before', _.map(solid.csg.polygons, p => {return p.isConvex()}))
                ...
                // _.forEach(solid.csg.polygons, p => {
                //         p.convex = p.isConvex()
                // })
                ...
                // HACK!!
                solid.csg.polygons[0].convex = isConvex
                // if (!solid.csg.polygons[0].isConvex()){
                //     console.log(solid.csg.polygons[0])
                // }
                // console.log(solid.parent.name, _.map(solid.csg.polygons, p => {return p.isConvex()}))
