...
function align_one_dimension(solids, dim, v) {
    ...
    _.rest(solids).forEach(function(solid) {
        ...
        var newLoc = _.clone(solid.layout.position)
        var percent = v
        if (!isNaN(percent)) {
            newLoc[dim] = first.layout.position[dim] +
        ...
        if (matches.length > 0) {
            ...
            if (index === 0) {
                ...
                if (!isNaN(percent)) {
                    ...
                    delta[dim] = first.layout.position[dim]
                        + (first.layout.size[dim] - descendent.layout.size[dim]) * percent / 100
                        - descendent.layout.position[dim]

                }

                solid.transformAt(descendent).translate(delta.x, delta.y, delta.z)
            }
...
//
//             if (i == 0){
//                 d = ref.layout.position[dim] - spacing
//             } else {
//                 var delta = {x:0, y:0, z:0}
//                 delta[dim] = (d - ref.layout.size[dim]) - ref.layout.position[dim]
//                 solid.select(ref).translate(delta)
//                 d = d - ref.layout.size[dim] - spacing
//             }
//         })
...
function _parse_v2(code) {
    ...
    return _.map(toks, function(e,i) {
        ...
        if (m = e.match(/~((\-?[\d.]+,)*(\-?[\d.]+)?)~>/)){
            ...
            var spacing = _.map(m[1].split(','), function(d){
                return Number(d)
            })
