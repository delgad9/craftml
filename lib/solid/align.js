...
function align_v2(code, selector){
    ...
    _.forEach(ops, function(op) {
        ...
        if (op.name === 'lineup'){
            lineup_one_dimension_v2(solids, op.dim, op.spacing, op.direction)
        } else if (op.name === 'align'){
            align_one_dimension_v2(solids, op.dim, op.value)
        }
    })
...
function lineup_one_dimension_v2(solids, dim, spacing, direction){
    ...
    if (direction == 1){
        ...
        solids.forEach(function(solid, i) {
            if (i == 0){
                d = solid.layout.location[dim]
            } else {
                var delta = {x:0, y:0, z:0}
                delta[dim] = d - solid.layout.location[dim]
                solid.translate(delta)
            }
            d = d + solid.layout.size[dim] + spacing
        })
        ...
        solids.forEach(function(solid, i) {
            if (i == 0){
                ...
                var delta = {x:0, y:0, z:0}
                delta[dim] = (d - solid.layout.size[dim]) - solid.layout.location[dim]
                solid.translate(delta)
                d = d - solid.layout.size[dim]
            }
        })
...
function align_one_dimension_v2(solids, dim, v) {
    var first = solids[0]
    _.rest(solids).forEach(function(solid) {
