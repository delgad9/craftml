...
export function setDim(dim, value){
    let d = {x:0, y:0, z:0}
    d[dim] = value - this.layout.position[dim]
    this.translate(d.x,d.y,d.z)
}

export function land(){
    let dz = - this.layout.position.z
...
export function translateTo(loc){
    ...
    _.forEach(loc, function(v, dim){
        d[dim] = v - this.layout.position[dim]
    }, this)
...
export function scale(s) {
    // with respect to the object center

    var center = {
        x: this.layout.position.x + this.layout.size.x / 2,
        y: this.layout.position.y + this.layout.size.y / 2,
        z: this.layout.position.z + this.layout.size.z / 2
    }
...
export function resize(newSize) {
    ...
    // var savedLoc = _.clone(this.layout.position)
...
export function mirror(dim, offset){
    ...
    if (arguments.length === 2){
        ...
        w = - this.layout.position[dim] - this.layout.size[dim] / 2
    }
...
// function cropLayoutRecursively(solid){
//
//     let box = solid.layout.clone()//c.layout.clone()
//     console.log(solid.element.name, 'before box:', box.position, box.size)
//     // box.transform(solid.m)
//
//     // console.log(solid.element.name, 'after box:', box.position, box.size)
...
//         }
//
//         console.log('  c', c.element.name, 'p:', c.layout.position, 's:', c.layout.size)
//         c.layout.transform(solid.m)
//         console.log('  c0', c.element.name, 'p:', c.layout.position, 's:', c.layout.size)
//
//         _.forEach(['x','y','z'], dim => {
//
//             let s1 = box.position[dim]
//             let s2 = box.position[dim] + box.size[dim]
//
//             let c1 = c.layout.position[dim]
//             let c2 = c.layout.position[dim] + c.layout.size[dim]
//
//             c.layout.position[dim] = Math.max(c1,s1)
//             c.layout.size[dim] = Math.min(c2,s2) - c.layout.position[dim]
//         })
//
//         console.log('  c1', c.element.name, 'p:', c.layout.position, 's:', c.layout.size)
//
//         if (c.role != 'define'){
//             //console.log('  c', c.element.name, 'p:', c.layout.position, 's:', c.layout.size)
//             c.layout.transformTo(c.layout.position, c.layout.size)
//             cropLayoutRecursively(c)
//         }
//     })
// }
...
TransformSelectedOp.prototype.translate = function(){
    // var loc = _parse_arguments_to_position(arguments)
