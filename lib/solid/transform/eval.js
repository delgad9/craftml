...
let valid = /(translate|translateTo|rotate|tighten|scale|scaleTo|fit|center|centerAt|mirror|crop|apply|set|land|resize)[XYZ]*/
...
export default function transformEval(code) {
    ...
    _.forEach(ops, op => {
        ...
        let f = this[op.name]
        if (f && op.name.match(valid)){

            f.apply(this, args)

        } else {

            throw `"${op.name}" is not a valid transform command`
        }
